...
Route::group(['namespace' => 'Web', 'middleware' => ['check_mobile_app', 'impersonate', 'share', 'check_maintenance']], function () {
    ...
    Route::group(['middleware' => 'web.auth'], function () {
        ...
        // تم اخفاء file manager وامكانياته لإن تم إختراق الموقع ورفع مثلفات dots بسببه مثل .htaccess
        ...
        Route::group(['prefix' => 'cart'], function () {
            Route::get('/', 'CartController@index')->name('cart.index.view');

            Route::post('/coupon/validate', 'CartController@couponValidate');

            /* Route move user with order data from cart page to checkout page */
    ...
    Route::group(['prefix' => 'payments'], function () {
        ...
        Route::get('/tabby-message/{status}', ['as' => 'tabby_message', 'uses' => 'PaymentController@tabbyMessage']);
        ...
        Route::get('/tamara/webhook', 'PaymentController@tamaraWebhook');
        Route::post('/tamara/webhook', 'PaymentController@tamaraWebhook');
        Route::post('/tabby/webhook', 'PaymentController@tabbyWebhook');
    });
...
Route::get('/troubleshooting', function () {
    ...
    foreach($Sales as $Sale){
        ...
        $getTmpSale = DB::table('tmp_sales')->where('user_id',$Sale->buyer_id)->first();
        ...
        $getDupsCount = DB::table('sales')->where('buyer_id',$Sale->buyer_id);

        if($Sale->webinar_id){
            $getDupsCount = $getDupsCount->where('webinar_id',$Sale->webinar_id);
        }
        if($Sale->bundle_id){
            $getDupsCount = $getDupsCount->where('bundle_id',$Sale->bundle_id);
        }

        $getDupsCount = $getDupsCount->count();
        ...
        // Fill payment method
        DB::table('sales')->where('id',$Sale->id)->update(['online_payment_method' => $getTmpSale->online_payment_method,'online_payment_method_id' => $getTmpSale->online_payment_method_id]);
...
WHERE  NOT EXISTS
  (SELECT *
   FROM   sales
   WHERE  sales.order_id = orders.id)`);
...
Route::prefix('panel')->name('panel.')->middleware(['auth'])->group(function () {
    ...
    // grade edit/update/delete for teacher UI
    Route::get('grades/{id}/edit', [TermGradesController::class, 'editGrade'])
        ->name('panel.grades.edit');

    Route::patch('grades/{id}', [TermGradesController::class, 'updateGrade'])
        ->name('panel.grades.update');

    Route::delete('grades/{id}', [TermGradesController::class, 'deleteGrade'])
