...
Route::group(['namespace' => 'Panel', 'prefix' => 'panel', 'middleware' => ['check_mobile_app', 'impersonate', 'panel', 'share', 'check_maintenance']], function () {
    ...
    Route::group(['prefix' => 'webinars'], function () {
        ...
        Route::group(['middleware' => 'user.not.access'], function () {
            ...
            /* رؤية الطلاب في الكورس الواحد */
            Route::group(['prefix' => '{webinar_id}/ShowStudents'], function () {
                Route::get('/', 'WebinarStatisticController@ShowStudents');
                Route::get('/new','WebinarStatisticController@addStudent');
                Route::get('/new/excel', 'WebinarStatisticController@addStudentViaExcel');
//                Route::post('/store/Excel', 'WebinarStatisticController@addStudent');
            });
        ...
        Route::get('/term_grades', 'TermGradesController@termGrades');
        Route::get('/add_term_grades', 'TermGradesController@termGradesShowCreate');
        Route::post('/term_grades', 'TermGradesController@termGradesStore');
        Route::get('/student_grades', 'TermGradesController@studentGrades');
        
     Route::prefix('panel')->name('panel.')->middleware(['auth'])->group(function () {
        // list
        Route::get('webinars/term_grades', [TermGradesController::class, 'termGrades'])
            ->name('webinars.term_grades.index');
    
        // show create form
        Route::get('webinars/add_term_grades', [TermGradesController::class, 'termGradesShowCreate'])
            ->name('webinars.add_term_grades');
              Route::get('webinars/{webinar}/students', [TermGradesController::class, 'studentsForWebinar'])
            ->name('panel.webinars.students');
    
        // store grades (this provides the route name your blade expects)
        Route::post('webinars/term_grades', [TermGradesController::class, 'termGradesStore'])
            ->name('webinars.store_term_grades');
