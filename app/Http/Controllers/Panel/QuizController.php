...
class QuizController extends Controller
{
    ...
    //          'show_result_to_student' => (!empty($data['show_result_to_student']) and $data['show_result_to_student'] == 'on'),
    ...
    public function store(Request $request)
{
    ...
    // لو ما في اختيارات، نحط مصفوفة فاضية
    $webinarIds = $data['webinar_ids'] ?? [];

    $createdQuizIds = []; 

    foreach ($webinarIds as $webinarId) {
        // نتأكد أن المستخدم له صلاحية على الويببنار
        ...
        if (empty($webinar)) {
            continue; // يتجاوز أي ويببنار ما يخص المستخدم
        }
    ...
    Quiz::whereIn('id', $createdQuizIds)->update([
    'quiz_ids' => $createdQuizIds,
]);
    ...
    if ($request->ajax()) {
        return response()->json([
            'code' => 200,
            'redirect_url' => '/panel/quizzes',
            'message' => 'تم إنشاء الكويزات لجميع الويبنارات المختارة بنجاح.'
        ]);
    } else {
        return redirect('/panel/quizzes')->with('success', 'تم إنشاء الكويزات بنجاح.');
    }
}
