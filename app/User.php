...
class User extends Authenticatable
{
    ...
    public function isUserBoughtEnglish(){
        return \App\Models\Sale::where('webinar_id',2035)->where('buyer_id',auth()->user()->id)->count() > 0;
    }
    ...
    public function bundleWebinars(): HasMany
    {
        return $this->hasMany(BundleWebinarStudent::class, 'user_id');
    }

    /**
     * getting the total hours registered in the given semester
     */
    public function getTotalRegisterdHours($semester_id = null, $bundle_id = null): int
    {
        $webinar_hours = 0;
        $bundle_webinars = $this->bundleWebinars()
            ->when($bundle_id, function ($query) use ($bundle_id) {
                $query->whereHas('bundleWebinar', function ($bundleWebinar) use ($bundle_id) {
                    $bundleWebinar->where('bundle_id', $bundle_id);
                });
            })->when($semester_id, function ($query) use ($semester_id) {
                $query->whereHas('bundleWebinar', function ($bundleWebinar) use ($semester_id) {
                    $bundleWebinar->where('bundle_semester_id', $semester_id);
                });
            })->get();
        foreach ($bundle_webinars as $bundle_webinar_item) {
            $webinar_hours += $bundle_webinar_item->bundleWebinar->webinar?->number_of_hours ?? 0;
        }
        return $webinar_hours;
    }
