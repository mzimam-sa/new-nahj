...
@php
    ...
    $isTeacher = $user && (($course->teacher_id == $user->id) || (method_exists($course, 'isPartnerTeacher') && $course->isPartnerTeacher($user->id)));
    $isReviewer = $user && $course->reviews->where('creator_id', $user->id)->count() > 0;
@endphp

@if($isAdmin || $isTeacher || $isReviewer)
    <div class="mt-35">
...
{{-- إظهار التقييمات للطالب صاحب التقييم، المدرس، الأدمن فقط --}}
        ...
        @if($course->reviews->count() > 0)
            @foreach($course->reviews as $review)
                @php
                    $isAdmin = $user && method_exists($user, 'isAdmin') ? $user->isAdmin() : false;
                    $isTeacher = $user && (($course->teacher_id == $user->id) || (method_exists($course, 'isPartnerTeacher') && $course->isPartnerTeacher($user->id)));
                    $isOwner = $user && $review->creator_id == $user->id;
                    $canSeeReview = $isAdmin || $isTeacher || $isOwner;
                @endphp
                @if($canSeeReview)
                <div class="comments-card shadow-lg rounded-sm border px-20 py-15 mt-30" data-address="/reviews/store-reply-comment" data-csrf="{{ csrf_token() }}" data-id="{{ $review->id }}">
                    <div class="d-flex align-items-center justify-content-between">
                        <div class="user-inline-avatar d-flex align-items-center mt-10">
                            ...
                            <div class="d-flex flex-column ml-5">
                                <span class="font-weight-500 text-secondary mb-2 mr-2">{{ $review->creator->full_name }}</span>
                                @include(getTemplate() . '.includes.webinar.rate',[
                                    'rate' => $review->rates,
                                    'dontShowRate' => true,
                        ...
                        <div class="d-flex align-items-center">
                            <span class="font-12 text-gray ml-10">{{ dateTimeFormat($review->created_at, 'j M Y | H:i') }}</span>
                    ...
                    @if($review->comments->count() > 0)
                        @foreach($review->comments as $comment)
                            @php
                                $isCommentOwner = $user && $comment->user_id == $user->id;
                                $canSeeComment = $isAdmin || $isTeacher || $isCommentOwner || $isOwner;
                            @endphp
                            @if($canSeeComment)
                            <div class="shadow-lg rounded-sm border px-20 py-15 mt-30">
                                <div class="d-flex align-items-center justify-content-between">
                                    <div class="user-inline-avatar d-flex align-items-center mt-10">
                                        ...
                                        <div class="d-flex flex-column ml-5">
                                            <span class="font-weight-500 text-secondary mb-2 mr-2">{{ $comment->user->full_name }}</span>
                                        </div>
                                    </div>
                                    <div class="d-flex align-items-center">
                                        <span class="font-12 text-gray ml-10">{{ dateTimeFormat($comment->created_at, 'j M Y | H:i') }}</span>
                                        <div class="btn-group dropdown table-actions">
                                            <button type="button" class="btn-transparent dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                <i data-feather="more-vertical" height="20"></i>
                                            </button>
