    ...
    <section class="container course-content-section {{ $course->type }} {{ ($hasBought or $course->isWebinar()) ? 'has-progress-bar' : '' }}">
        <div class="row">
            <div class="col-12 col-lg-8">
                <div class="course-content-body user-select-none mt-80">
                    <div class="course-body-on-cover text-white">
                        ...
                        {{-- إخفاء الجزء الخاص بعرض شريط البروجرس والطلاب --}}
                    ...
                    {{-- القسم المسئول عن عرض تفاصيل الكورس --}}
                    <div class="mt-80">
                        <ul class="nav nav-tabs bg-white rounded-sm p-15 d-flex align-items-center justify-content-between" id="tabs-tab" role="tablist">
                            <li class="nav-item">
                                <a class="position-relative font-14 text-secondey {{ (empty(request()->get('tab','')) or request()->get('tab','') == 'information') ? 'active' : '' }}" id="information-tab"
                                   data-toggle="tab" href="#information" role="tab" aria-controls="information"
                                   aria-selected="true">{{ trans('product.information') }}</a>
                            </li>
                            @if($webinarContentCount)
                            <li class="nav-item">
                                <a class="position-relative font-14 text-secondey {{ (request()->get('tab','') == 'content') ? 'active' : '' }}" id="content-tab" data-toggle="tab"
                                   href="#content" role="tab" aria-controls="content"
                                   aria-selected="false">{{ trans('product.content') }} ({{ $webinarContentCount }})</a>
                            </li>
                            @endif
                            <li class="nav-item">
                                <a class="position-relative font-14 text-secondey {{ (request()->get('tab','') == 'reviews') ? 'active' : '' }}" id="reviews-tab" data-toggle="tab"
            ...
            <div class="course-content-sidebar col-12 col-lg-4 mt-25 mt-lg-0">
                <div class="rounded-lg shadow-sm">
                    ...
                    <div class="px-20 pb-30">
                        <form action="/cart/store" method="post">
                            ...
                            <!-- Start Installment Banner -->
                             <div class="bundle-installment-banner">
                             يوجد اقساط ميسرة عن طريق التواصل مع المعهد
                             <hr>
                            <div>
                                <a class="d-flex align-items-center justify-content-center text-gray font-16" href="tel:{{ $generalSettings['site_phone'] }}">
                        ...
                        <div class="mt-35">
                            <strong class="d-block text-secondary font-weight-bold">المحتوى التعليمي يشمل:</strong>
                            ...
                            {{-- @if($course->start_date)
                                <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                                    ...
                                    <span class="font-14">{{ dateTimeFormat($course->start_date, 'd M Y') }}</span>
                                </div>
                            @endif --}}
                ...
                {{-- تعطيل جزء المدرس في حالة الاوفلاين --}}
                {{--
...
@push('scripts_bottom')
    @if(App\PaymentChannels\Drivers\Tabby\Channel::showTabby($course->price))
        <div id="tabby"></div>
        ...
        price: '{{ $course->price }}', // required, price of the product. 2 decimals max for AED|SAR and 3 decimals max for KWD.
        lang: 'ar', // Optional, en|ar only supported
        source: 'product', // Optional, snippet placement; `product` for product page and `cart` for cart page.
        publicKey: '{{ env('TABBY_PUBLIC_KEY') }}', // required, Public Key
        merchantCode: 'nahj'  // required
        });
        </script>
    @endif
