...
<div class="learning-page-navbar d-flex align-items-lg-center justify-content-between flex-column flex-lg-row px-15 px-lg-35 py-10">
    ...
    <div class="d-flex align-items-center mt-5 mt-md-0">
        ...
        @if(!empty($course->noticeboards_count) and $course->noticeboards_count > 0)
            <a href="{{ $course->getNoticeboardsPageUrl() }}" target="_blank" class="btn learning-page-navbar-btn noticeboard-btn btn-sm border-gray200 ml-10">
        ...
        @if($course->forum)
            <a href="{{ $course->getForumPageUrl() }}" class="btn learning-page-navbar-btn btn-sm border-gray200 ml-10">{{ trans('update.course_forum') }}</a>
        @endif

        <div class="d-none align-items-center d-lg-flex">
            <a href="{{ $course->getUrl() }}" class="btn learning-page-navbar-btn btn-sm border-gray200">عن المحتوى التعليمي</a>

            <a href="/panel/webinars/purchases" class="btn learning-page-navbar-btn btn-sm border-gray200 ml-10">مشترياتي</a>
        </div>
